...
fn main() {
    ...
    let t1_partition = xs[0].clone();
    let t2_partition = xs[1].clone();

    let t1 = thread::spawn(move || map_data(&t1_partition));
    let t2 = thread::spawn(move || map_data(&t2_partition));
    ...
    let mt_partitions = partition_data(num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&mt_partitions);

    // 3. Creates one thread per partition and uses each thread to concurrently process one partition
    let mut mt_threads: Vec<thread::JoinHandle<usize>> = Vec::new();
    for i in 0..num_partitions {
        let p = mt_partitions[i].clone();
        mt_threads.push(thread::spawn(move || map_data(&p)));
    }

    // 4. Collects the intermediate sums from all the threads
    let mut mt_intermediates: Vec<usize> = Vec::new();
    for t in mt_threads {
        let r = t.join().unwrap();
        mt_intermediates.push(r);
    }

    // 5. Prints information about the intermediate sums
    println!("MT intermediates = {:?}", mt_intermediates);

    // 5. Calls reduce_data to process the intermediate sums
    let mt_sum = reduce_data(&mt_intermediates);

    // 6. Prints the final sum computed by reduce_data
    println!("MT sum = {}", mt_sum);

    println!(
        "  Do ST and MT sums agree? {}",
        if sum == mt_sum { "YES" } else { "NO" }
    );
}
...
*/
fn partition_data(num_partitions: usize, data: &Vec<usize>) -> Vec<Vec<usize>> {
    // Create a vector that will contain vectors of integers
    let mut xs: Vec<Vec<usize>> = Vec::new();
    ...
    for i in 0..data.len() {
        xs[i % num_partitions].push(data[i])
    }
